...
| ✅ Trivy → Wazuh integration (platform component 14) | **Complete 2026-05-09** — CronJob + agent localfile + manager decoder/rules. 2227 alerts on first run (1989×HIGH, 217×CRITICAL, 20×CRITICAL-public-ns, all with MITRE T1190). Rules 100200–100203 levels 5/9/13/15 in `trivy_rules.xml`. Decoder pair `trivy`+`trivy-json` (JSON_Decoder) in `local_decoder.xml`. Manifests: `platform/manifests/trivy-system/{07,08}-trivy-wazuh-*.yaml` + `wazuh-agent/{03,04}-*.yaml`. Deploy: `platform/components/14-trivy-wazuh.sh`. Wazuh 4.14.4 findings documented as operator-backlog #20–24. |
...
| 🟨 Integrate Proposal Forge + Project Tracker (Phase 10) | **Started 2026-05-04**: 10.1.1 audit ✅, 10.1.2 SpiceDB schema additions ✅ (2026-05-05), 10.1.3 Postgres schema migration ✅ (2026-05-05 — 14 tables + 14 RLS policies in `project_tracker.*`, 23 rows imported, parity + RLS scoping verified). 10.1.4+ pending. |
...
> 18. ~~**Phase 7d.6 — Manager-side custom decoders for OpenBao audit JSON + Keycloak event JSON**~~ ✅ **Resolved 2026-05-05 (S3 audit cleanup).** `local_decoder.xml` updated on PVC with `openbao-audit` (program_name match + JSON_Decoder) and `keycloak-event` (program_name match + JSON_Decoder) decoder pairs. 8473 rules enabled; `wazuh-logtest` confirms actor/op/path fields populated for both shapes.
> 19. ~~**Trivy → Wazuh integration (Phase 14 component)**~~ ✅ **Deployed 2026-05-09 and verified end-to-end.** CronJob `trivy-wazuh-reporter` in `trivy-system` runs every 6h; writes traditional-syslog-prefixed NDJSON to `/var/log/trivy-wazuh/findings-*.jsonl` (hostPath). Agent DaemonSet ships new files via `<localfile log_format="syslog">`. Manager decodes via `trivy` (program_name=trivy-cve parent) + `trivy-json` (JSON_Decoder child) in `local_decoder.xml`. Rules 100200–100203 at levels 5/9/13/15. First verified run: 2834 findings written → 2227 Wazuh alerts (1989×HIGH, 217×CRITICAL, 20×CRITICAL-in-public-ns), all with MITRE T1190 on CRITICAL. Manifests: `platform/manifests/trivy-system/07-trivy-wazuh-rbac.yaml`, `08-trivy-wazuh-cronjob.yaml`, `wazuh-agent/03-configmap.yaml`, `wazuh-agent/04-daemonset.yaml`. Deployment script: `platform/components/14-trivy-wazuh.sh`.
> 20. **Wazuh 4.14.4 — SIGHUP does not reload custom decoders** (discovered 2026-05-09 during Trivy integration). `kill -HUP $(pgrep -x wazuh-analysisd)` reloads the rule set but silently ignores changes to `etc/decoders/`. After any decoder edit in `local_decoder.xml` or a new file under `etc/decoders/`, a full pod restart is required: `kubectl delete pod -n wazuh wazuh-manager-0`. `wazuh-logtest` (a separate binary that reads decoders fresh at startup) always shows correct behavior regardless; trust `wazuh-logtest` for decoder syntax validation, but redeploy the pod to make the running daemon pick up changes.
> 21. **Wazuh 4.14.4 — `only-future-events=no` ignored for non-empty files at discovery** (discovered 2026-05-09). When logcollector first discovers a glob-matched file that already contains data, it positions its offset at EOF and ships no historical content. The setting `<only-future-events>no</only-future-events>` only takes effect if the file is EMPTY at the moment of first discovery. Glob rescans happen every `logcollector.reload_interval=64` seconds (default). Consequence for the Trivy CronJob: `touch "$OUTFILE"; sleep 90` before writing ensures the 64s rescan cycle fires while the file is empty. If the sleep is reduced below 64s there is a race where logcollector may discover the file after content is written and skip all existing entries. The 90s sleep is load-bearing; do not reduce it without also lowering `logcollector.reload_interval` on the manager.
> 22. **Wazuh 4.14.4 — JSON_Decoder fields are at ROOT level (not `data.*`)** (discovered 2026-05-09). When a decoder uses `<plugin_decoder>JSON_Decoder</plugin_decoder>`, Wazuh extracts all top-level JSON keys directly as root-level fields. They are accessible as `$(severity)` (not `$(data.severity)`) in rule description templates and as `<field name="severity">` (not `<field name="data.severity">`) in rule match conditions. This is distinct from the `log_format=json` path in the agent (which also places fields under `data.*` in the manager's alert JSON). The two paths are NOT symmetric. Rule 100200–100203 in `trivy_rules.xml` use root-level names throughout.
> 23. **Wazuh syslog predecoder requires traditional timestamp format** (discovered 2026-05-09). RFC3339+Z timestamps (`2026-05-09T00:08:03Z`) are NOT recognized by Wazuh 4.14.4's syslog predecoder. Lines with that prefix produce `predecoder: {}` — no `hostname`, `program_name`, or `timestamp` extraction — so all parent decoders that match on `<program_name>` silently fail to fire. Lines must be prefixed with the traditional syslog format `MMM DD HH:MM:SS` (e.g., `May  9 00:36:09`, produced by `date -u +"%b %e %H:%M:%S"` in bash). This is why the CronJob uses `SYSLOG_TS=$(date -u +"%b %e %H:%M:%S")` as the line prefix and keeps the ISO `TS` only inside the JSON payload for machine-readable audit use.
> 24. **Wazuh OS_Regex syntax pitfalls** (documented 2026-05-09). Wazuh's OS_Regex engine (used in decoder `<regex>` and rule `<field>` values) differs from PCRE in two critical ways: (a) `.` is a **literal dot**, not "any char"; the wildcard is `\.`; (b) parentheses-grouped alternation `(foo|bar)` in `<field>` values causes rule loader error 5107 — use bare alternation `^foo$|^bar$` instead. These cost two debugging rounds in rule 100203 (`<field name="namespace">^(keycloak|ingress-nginx)$</field>` → error 5107; fixed to `^keycloak$|^ingress-nginx$`). If a rule silently never fires after a hot reload, OS_Regex syntax is the first thing to check via `wazuh-logtest`.
>
...
- ✅ **Wazuh OIDC federation with Keycloak** (Phase 7d Item 7 — scaffolding Complete 2026-05-02). Keycloak client `wazuh-dashboard` provisioned via `infrastructure/keycloak/clients/wazuh.sh` (kcadm-admin pattern; supersedes the grafana-style throwaway-client idiom). OpenBao policy + role + VSO binding at `platform/manifests/wazuh/03-vso-binding.yaml` render `wazuh-oidc-vso` Secret in wazuh ns. Auth-mode flip script `platform/components/07j-wazuh-oidc-configure.sh` (operator runs to switch dashboard from admin/admin to OIDC SSO; reversible). Skipped vendor-chart patching — in-place reconfig + securityadmin.sh push is the operator-time path.
...
- 10.1.3 — Postgres schema migration: ✅ Complete 2026-05-05 — `project_tracker.*` schema (14 tables) + 14 `tenant_isolation` RLS policies applied to `secforge-app-db`; AECOM-BD-Jason tenant UUID `833cc9ee-81b6-4e79-a4d7-e104fa37aa12` pinned at `apps/project-tracker/TENANT.md`; 23 rows imported from local docker-compose with `tenant_id` backfilled via sed-transform; `004-verify.sh` confirms parity (23 ⇄ 23 across 14 tables) AND RLS scoping (wrong-UUID → 0, AECOM-UUID → 3, GUC-unset → 0). OpenBao engine roles `project-tracker-readwrite` (1h) + `project-tracker-migrate` (15m) + policy + JWT auth role bound to SPIFFE ID provisioned via `infrastructure/project-tracker/provision-db-and-bao.sh`. PT-repo Prisma schema edited (PT commit `69f42a7`); platform repo gets the migration as `apps/project-tracker/migrations/001-prisma-baseline.sql` (verbatim copy).
- 10.1.4 through 10.1.10 — ⬜ Open
