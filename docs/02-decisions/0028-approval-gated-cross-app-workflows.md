...
**Recommended (MVP, avoids a PM schema expansion) — SIGNED OFF 2026-06-23:** on execute, PM seeds `wbs_nodes` from the PF deliverable tree (`ExtractedTask.taskNumber` → outline), then creates **group-level** labor `cost_estimates` per deliverable (hours = group total on that deliverable, hourly_rate = group blended base rate; DLM applied by PM as usual) — AND **stashes the full snapshot** (per-personnel backing, trip/sub detail, bid-vs-award) in `projects.custom_fields.sourceSnapshot` so nothing is lost and PM can surface/drill/reconcile. A dedicated PM labor-group/personnel-estimate schema is **deferred**. Everything stays editable; the award amount drives reconciliation.

**HARD REQUIREMENT (operator, 2026-06-23): the actual NON-LABOR dollar costs must land in the project budget, not just the snapshot stash.** PM execution creates **fixed `cost_estimates`** (one per line) for: **travel** (the computed trip TOTAL in $ — lodging+M&IE+airfare+ground, NOT hours), **subcontractors** (computed amount by cost type), and **additional costs / materials / assets / ODCs** (unitCost×qty). These attach under dedicated WBS nodes (e.g. `Travel`, `Subcontractors`, `Other Direct Costs`) so they roll into `totalEstimatedBudget` and the cost baseline alongside labor. The snapshot contract therefore carries `trips[]`/`subcontractors[]`/`additionalCosts[]` as `{name, scope, amount}` where **amount is the actual dollar figure** (the snapshot builder pulls it from PF's pricing summary, not raw hours).
