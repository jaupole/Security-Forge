...
| 6 | `HISTIGNORE` + `HISTCONTROL=ignorespace` in `~/.bashrc`; rewrite GHCR rotation flow to read PAT from sealed source, not stdin | [x] | `.bashrc` updated. **Rotation script committed 2026-06-09** — `platform/components/rotate-ghcr-pat.sh`: takes the PAT + an OpenBao token via silent `read -rs` prompts (never stdin-pipe / history / argv), writes the PAT to OpenBao `secret/apps/control/ghcr-pull` (over stdin, `password=-`), force-syncs every `ghcr-pull-secret` VaultStaticSecret, verifies. No root token needed. *Untested against a live rotation* — exercise on the next real PAT roll. |
...
| 14 | **H-4.8 audit tail — operator/controller-managed third-party images still on mutable tags.** velero + wazuh are now digest-pinned (`@sha256`, verified 2026-06-09), but `require-image-digest` (AUDIT) still flags: cert-manager suite (cert-manager / cainjector / webhook / trust-manager), CNPG operator-managed pods (`barman-cloud` + the `*-db-*` CNPG initContainers in control / proposal-forge / etc.), keycloak-operator, loki-canary, and trivy-operator's ephemeral scan pods. Either digest-pin per-operator (cert-manager Helm values, CNPG operator image config, trivy `trivy.imageRef`, …) or **accept + document** the audit posture for vendor/operator-managed images. | [ ] | **Audit-only, not blocking.** Many images are set by controllers/operators via tags the platform doesn't directly own; full pinning is a multi-touch effort. Recurring Warning events 2026-06-09. |
| 15 | Trim host DNS to ≤3 nameservers to clear `DNSConfigForming` warnings on coredns + node-exporter | [x] | **Done 2026-06-09.** The pod-facing `/run/systemd/resolve/resolv.conf` had **7** entries (global 3 + eno1's 4 per-link, with v4 dups). Removed the per-link `nameservers:` block from `/etc/netplan/01-netcfg.yaml` so eno1 falls back to the static global `DNS=185.12.64.2 185.12.64.1 9.9.9.9` in `/etc/systemd/resolved.conf` → `netplan apply` + `systemctl restart systemd-resolved` → uplink resolv.conf now **exactly 3**; external resolution verified (`resolvectl query github.com` OK), coredns unaffected. **cloud-init is NOT installed** on the box → no per-boot regeneration, so it persists across reboot. Backup at `/etc/netplan/01-netcfg.yaml.bak-2026-06-09`. (Existing pods keep their old config until next restart.) |

## Disk hygiene

| Date | Action | Result | Next |
|------|--------|--------|------|
| 2026-06-09 (eve) | Live re-check during session cleanup | root `/` **62%**, rancher 45%, minio 28%, wazuh 34%, cnpg 1%; node Ready, no failing pods | runner-cleanup cron confirmed **installed** (`/etc/cron.d/runner-cleanup`, root 644, verified 2026-06-09); healthy headroom |
...
| 2026-06-09 | Added `platform/host/runner-cleanup.cron` (daily 03:30 UTC) — prunes `_work` dirs >1d + `.cache`/`.npm`/`.local` dirs >7d + `docker volume prune -f` + `docker builder prune --keep-storage 500mb`. **Installed 2026-06-09.** | In `main` (the `fleet/w4` branch was merged + pruned 2026-06-09) | ✅ Installed — verified `/etc/cron.d/runner-cleanup` (root 644). Self-managing. |
