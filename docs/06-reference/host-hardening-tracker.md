...
| 15 | Trim host `/etc/resolv.conf` to ≤3 nameservers to clear `DNSConfigForming` warnings on coredns + node-exporter | [ ] | Cosmetic; k8s caps resolv.conf at 3 nameservers and omits the rest (currently `185.12.64.2 185.12.64.1 9.9.9.9` applied). Verify + persist the trim at the source (cloud-init/netplan/Hetzner) so it survives reboot. |
