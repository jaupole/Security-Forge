...
| 2026-06-09 | Added `platform/host/runner-cleanup.cron` (daily 03:30 UTC) — prunes `_work` dirs >1d + `docker volume prune -f`. **Not yet installed.** | Staged on `fleet/w4` | Install: `sudo cp platform/host/runner-cleanup.cron /etc/cron.d/runner-cleanup` |
...
Root causes: (a) `/opt/github-runner` (18 GB) — CI runner `_work` dirs + `.cache`/`.npm` grow without bound; automated by runner-cleanup cron above. (b) `/var/lib/docker/volumes` (8.9 GB) — Docker anonymous CI volumes never pruned; same cron covers it. (c) `/var/lib/rancher/k3s/agent/containerd` (53 GB) — stale digest-pinned app images accumulate per deploy; covered by kubelet GC thresholds.
