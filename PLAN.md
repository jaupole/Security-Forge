...
> Last updated: 2026-05-02 (Session 5 — **Phase 7d.1 + 7d.2 + Items 3 & 4 ✅ COMPLETE.** Plus operator-backlog #4 + #16 resolved. Items 5-7 (Wazuh trio) still deferred. 7d.1 ships `infrastructure/keycloak/realms/rotate-bff-key.sh` + four staggered CronJobs (`bff-key-rotator-<bff>` in `app` ns; only `helloworld-bff` enabled, others suspend:true until Phase 9+) + runbook `docs/03-runbooks/bff-key-rotation.md`; manual rotation of `helloworld-bff` verified end-to-end. 7d.2 ships [ADR-0023](./docs/02-decisions/0023-spicedb-datastore-uri-rotation-pattern.md) (Path B — CronJob-refreshed VaultStaticSecret) + `spicedb-datastore-refresher` 12h CronJob; verified end-to-end. **Item 3** switched the Transit unseal token from `-ttl=24h -renewable=true` to `-period=720h` periodic (auto-refreshes on transit-unseal call at boot; 30d cold-pause ceiling vs prior 24h); ADR-0009 + runbooks updated; operator-backlog #4 closed. **Item 4** swapped `unauthenticated_metrics_access=true` for bearer-token auth: `metrics-policy.hcl` + `configure-metrics-auth.sh` mints periodic token into `openbao/openbao-metrics-token` K8s Secret (must be same-ns as ServiceMonitor — discovered during apply, fixed); listener flipped + pods rolled in 2→1→0; verified all 3 Prometheus targets `health: up` post-cutover. **Operator-backlog #16** (Phase 5.7 `secforge-app` DB-engine rot) re-bootstrapped — `helloworld-app-readwrite` now mints cleanly. **Background**: scheduled remote agent `trig_01TdbXs13jkY8jyLsfJuHEMM` to fire 2026-07-08 with verification checklist for the first cron-driven rotation.)
...
| 🟨 7d — Rotation + housekeeping batch | **7d.1 + 7d.2 + Items 3 & 4 ✅ Complete 2026-05-02** (BFF private_key_jwt rotation runbook + script + 4 CronJobs; SpiceDB datastore_uri Path B per [ADR-0023](./docs/02-decisions/0023-spicedb-datastore-uri-rotation-pattern.md); Transit token periodic-720h migration; OpenBao metrics bearer-token auth). **Items 5–7 still deferred** as Phase 7.2 follow-ups (Wazuh agent hardening / syslog forwarding / OIDC federation). |
                                ...
                                └─→ Phase 6 (Istio + BFF) ✅
                                      ...
                                      └─→ Phase 7 (Observability) ✅
                                            ...
                                            └─→ 7.1–7.10 (stack) ✅
                                                  ...
                                                  └─→ Phase 7d (Rotation + housekeeping) 🟨 2026-05-02 (7d.1 + 7d.2 prompt-doc scope ✅; Items 3 + 4 ✅; Items 5–7 = Wazuh trio still deferred)
                                                        ↓
