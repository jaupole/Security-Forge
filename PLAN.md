                                ...
                                └─→ Phase 6 (Istio + BFF) ✅
                                      ...
                                      └─→ Phase 7 (Observability) ✅
                                            ...
                                            └─→ 7.1–7.10 (stack) ✅
                                                  ...
                                                  ├─→ Phase 7c (SPIRE-as-CA + STRICT) ⬜ DEFERRED 2026-05-02 — gating 7c.0.1 pre-flight failed (Ambient AuthZ-denial observability gap; tracked as operator-backlog #15)
                                                ...
                                                ☆ Fix-after-07 (this package) ✅ Complete 2026-05-01 (six signed commits; tagged `fix-after-07-complete`) ☆
                                                  ...
                                                  └─→ Phase 9 (Hello World) ⬜
                                                          [needs: 1-7 ✅, 6b-1 ✅, 3 follow-up ✅, Fix-after-07 ✅, F-CLU-11 ✅ — all gates closed]
...
> 15. **Istio Ambient AuthZ-denial observability gap** (surfaced 2026-05-02 during Phase 7c.0.1 pre-flight; **Phase 7c blocker**). On ztunnel 1.29.2 in this cluster, an AuthorizationPolicy denial (probe pod in `default` ns → `authzen-facade.app:8080`, blocked by `default-deny` + non-matching ALLOW) is correctly rejected at L4 (wget times out), but **no log line surfaces** in ztunnel logs at info OR debug level — only `"connection complete"` entries for successful connections. ztunnel Prometheus metrics expose `response_flags` on `istio_tcp_connections_opened_total` but every counter currently shows `response_flags="-"`; no separate denial counter is incrementing. Phase 7c's prompt § 7c.0.1 makes denial visibility a **gating** pre-flight check ("If denials are NOT visible in Loki: stop. Without visibility, the STRICT flip is unrecoverable when something breaks"); Phase 7c is paused on this until the gap closes. **Investigation paths:** (a) check Istio Ambient docs for an explicit log-emission flag (e.g. `pilot.env.PILOT_ENABLE_AMBIENT_AUTHZ_LOG` / equivalent) that turns on per-decision logging on ztunnel, (b) inspect ztunnel source for the L4-policy-decision code path to confirm whether logging is feature-flagged or simply not implemented for the non-mesh-source case, (c) trigger denials from a mesh-resident pod (so source-side ztunnel sees the connection too) and check whether dst-side observability differs, (d) check whether `istio_requests_total` (HTTP-layer counter, requires waypoint) increments separately from `istio_tcp_connections_opened_total` for HTTP-tier denials. Effort: ~1-3 hours of focused investigation. Until this is fixed, **strategy A namespace-by-namespace cutover with working/not-working signals as the substitute for log observability** is the only safe Phase 7c path; strategy B (all-at-once flip) is foreclosed.
>
...
## Phase 7c — Istio SPIRE-as-CA cutover + PeerAuthentication STRICT *(1-2 days)*
**Status: ⬜ DEFERRED 2026-05-02 — 7c.0.1 pre-flight gate failed (Ambient AuthZ-denial observability gap, tracked as operator-backlog #15). Resume once the gap is closed.**

> **2026-05-02 attempt — gate finding (7c.0.1):** triggered a known AuthZ denial (probe pod in `default` ns → `authzen-facade.app:8080`, which has a `mesh-authz=enforce` selector + ALLOW only from `app` ns); the connection was correctly rejected at the L4 layer (wget timed out), confirming the policy fired. **However:** ztunnel logs at info AND debug levels emit `"connection complete"` only for **successful** connections; no `"RBAC: access denied"` / policy-decision / rejection event surfaces for the denied path. ztunnel's Prometheus metrics (`/15020/metrics`) expose `istio_tcp_connections_opened_total` with `response_flags` and full source/dest principal labels, but every counter on this cluster has `response_flags="-"` — denials don't increment a separate counter visible to Prometheus today. The prompt's gating instruction is explicit: **"If denials are NOT visible in Loki: stop. Without visibility, the STRICT flip is unrecoverable when something breaks."** Per that, Phase 7c is paused. Investigation of upstream-Istio-Ambient denial-observability paths (response_flag emission config, `pilot.env.PILOT_ENABLE_AMBIENT_AUTHZ_LOG` or equivalent, alternative metrics) is filed as operator-backlog #15.
...
**Deliverables:**
- **BFF `private_key_jwt` rotation runbook** (`docs/03-runbooks/bff-private-key-jwt-rotation.md`): step-by-step procedure for rotating the per-client RSA-2048 signing keys in `secret/data/keycloak/clients/<id>` against the corresponding Keycloak client's JWKS
- **90-day rotation cron**: scheduled CronJob that triggers the runbook procedure (or surfaces a runbook-ready PR)
- **SpiceDB `datastore_uri` static-copy → database-engine migration**: extend OpenBao's database secrets engine (already wired for `helloworld-app` in Phase 5.7) to issue dynamic Postgres creds for SpiceDB; replace the `VaultStaticSecret` with a `VaultDynamicSecret`; SpiceDB Operator's `secretName` keeps pointing at the rendered Secret, content becomes dynamic. Resolves the CNPG-rotation desync risk documented in ADR-0015.
- **Transit unseal token TTL strategy for cold-pause clusters**: the 24h renewable TTL assumes the main OpenBao is up most of the time so auto-renewal fires. On a part-time local cluster that may sit cold for days (hit 2026-05-01: cluster paused → 403 permission denied on next start, full Shamir + token rotation required). Options to evaluate: (a) longer TTL (e.g. 720h) with explicit periodic rotation, (b) `period` instead of `ttl` so renewal isn't required, (c) script-driven mint on every cluster bring-up. Pick one; document tradeoffs in updated [openbao-recovery.md](./docs/03-runbooks/openbao-recovery.md) and [ADR-0009](./docs/02-decisions/0009-openbao-seal-strategy.md).
- **OpenBao metrics auth pre-cloud-migration**: Session 3 (2026-05-01) shipped `unauthenticated_metrics_access = true` on the OpenBao listener as a local-edition simplification, gated by the `allow-prometheus-to-openbao-metrics` NetworkPolicy. Before any cloud promotion: (1) create a `metrics-policy` token with capabilities only on `sys/metrics`, (2) inject it into a Secret consumed by the ServiceMonitor `bearerTokenSecret`, (3) flip the listener block to `unauthenticated_metrics_access = false`, (4) ensure cert SANs match the in-cluster Service DNS so `insecureSkipVerify` can be removed, (5) verify scrape still works end-to-end. Inline TODO is in `infrastructure/openbao/04-openbao-values.yaml` listener block.
- **Wazuh agent hardening + redeploy** (added Session 4, 2026-05-01): Phase 7.2 deferred the agent DaemonSet because the chart's default sets `securityContext.privileged: true` + `hostPID: true` + hostPath mounts of `/etc`, `/var/log`, `/proc`. Reintroduce with: (1) image pinned by SHA256 digest (not tag), (2) `privileged: true` replaced by explicit `capabilities.add: [SYS_PTRACE, AUDIT_READ, AUDIT_CONTROL]` minimal set, (3) `hostPID: true` only if strictly required (test FIM/syscheck without it first), (4) per-SA exemption from `pod-security.kubernetes.io/enforce: baseline` rather than relaxing the whole `wazuh` namespace to `privileged`. Patch the vendored chart's agent DaemonSet template; document in `vendor/PATCHES.md` (alongside P-001 NET_RAW removal and P-002 nodeSelector removal). Companion runbook update in `docs/03-runbooks/wazuh-operations.md` "Deferred components" → mark complete.
- **Wazuh — Keycloak/OpenBao → manager syslog forwarding** (added Session 4): Phase 7.2 NetworkPolicy `allow-syslog-to-wazuh-manager` is in place. Source-side wiring needs: (a) OpenBao `bao audit enable syslog address=wazuh-manager.wazuh.svc:1514` (non-destructive runtime change), (b) Keycloak `spi-events-listener-syslog-*` Quarkus options in `infrastructure/keycloak/04-keycloak-cr.yaml` (requires Keycloak roll). Plus custom decoders in Wazuh manager (`/var/ossec/etc/decoders/local_decoder.xml`) for OpenBao audit JSON and Keycloak event JSON. Verification: `verify-e2e.sh` 5th-pillar check upgraded to "Wazuh manager has indexed events from openbao+keycloak in last 1h."
- **Wazuh OIDC federation with Keycloak** (added Session 4): create `wazuh-dashboard` Keycloak client via `infrastructure/keycloak/clients/wazuh.sh` (Path A, mirror `grafana.sh`); configure OpenSearch Security plugin (`config.yml` + `roles_mapping.yml`) to accept Keycloak OIDC tokens; map `platform_admin` realm role → Wazuh `admin` backend role. Approach: chart values `dashboard.config` override + a separate ConfigMap for OpenSearch Security plugin config. Estimate: 60-90 min once we sit down for it.
- Verification: rotate a BFF key end-to-end without service disruption; verify SpiceDB CheckPermission still works after a forced CNPG password rotation (no manual re-run of the static migration script needed); cold-pause cluster for 48h+ then bring up clean without manual token rotation; Wazuh agent reinstated under PSS=baseline + caps-only hardening; Keycloak/OpenBao events reaching Wazuh indexer; OIDC login to Wazuh dashboard working with `platform_admin` realm role.

**See:** [docs/05-claude-code-prompts/phase-07d-rotation-housekeeping.md](./docs/05-claude-code-prompts/phase-07d-rotation-housekeeping.md); Phase 5 follow-ups #4 and Phase 6 follow-up #3; [ADR-0015](./docs/02-decisions/0015-secret-distribution-pattern.md) §"What we did NOT do."
...
**Checkpoint sign-off (fill in before proceeding to 9.11):**
- [ ] Date checkpoint passed: __________
