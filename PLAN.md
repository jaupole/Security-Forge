...
> Last updated: 2026-05-01 (Session 4 — **7.2 ✅ Wazuh deployed.** All 3 pods (indexer + manager + dashboard) Ready 1/1; indexer cluster green; dashboard reachable at https://wazuh.secforge.local/app/login. Agent DaemonSet deferred to Phase 7d; OIDC + log forwarding tracked as 7.2 follow-ups. Phase 7 fully closed except for the 7-day SPIFFE-CSI soak running in background. **6b-0 follow-up relocated to "Watching briefs" subsection below the phase table** — it's a re-evaluation condition, not a phase to execute. **🚨 Git initialization gap discovered during Phase 6b-1 docs work** — added to operator backlog as item #6 (HIGH PRIORITY); ADR-0021 stub reserved at `docs/02-decisions/0021-git-initialization-and-commit-signing.md`; kcadm-admin ADR slot moves to 0022. **6b-1 unblocked (2026-05-01):** ADR-0012 § Resolution appended with Q1-Q4 decisions; ADR-0014 fleshed out from stub to full library design contract; `phase-06b-api-pattern.md` rewritten as 6b-1-only runnable prompt; `phase-06b-2-outbound-secrets.md` created to preserve the (never-stale) 6b-2 content. Status flips ⏸️ → ⬜.)
...
| ⬜ ☆ Fix-after-07 remediation package | runs after 7 ✅; see [Fix after 07/](./Fix%20after%2007/) |
...
> 6. **🚨 Git initialization (HIGH PRIORITY — discovered 2026-05-01 during Phase 6b-1 docs work)** — the project has NEVER been git-initialized. `git rev-parse --is-inside-work-tree` returns `fatal: not a git repository`. This blocks: (a) `Fix after 07/01-fix-prompt.md` Section §0.3 (`git checkout -b fix-after-07`), (b) commit-per-section structure throughout the fix prompt, (c) Phase 9+ app development (cannot build apps without source control), (d) commit signing when the supply-chain phase lands. **Must land before Fix-after-07 runs and before Phase 9.** Decision-set: see new [ADR-0021 stub](./docs/02-decisions/0021-git-initialization-and-commit-signing.md) covering: `.gitignore` (Shamir keys, OpenBao recovery PEMs, mkcert files, `.claude/settings.local.json`, `notes/`), commit author identity, GPG signing, initial commit strategy (single big commit vs. backfilled phase-by-phase history), remote (private GitHub / self-hosted Gitea / local-only), pre-commit hooks (gitleaks for secret detection per `templates/app-repo/.pre-commit-config.yaml` reference). ~1–2 hour operator session.
>
