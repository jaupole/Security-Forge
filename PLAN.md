...
| ⬜ 6b-1 — API Auth Pattern | unblocked 2026-05-01 — ADR-0012 § Resolution + ADR-0014 published; prompt rewritten as runnable. Audience-at-login model per Q1-Q4 decisions. |
...
**Status: ⬜ Ready to execute** — unblocked 2026-05-01 (Session 4) when the four open design questions from [ADR-0012](./docs/02-decisions/0012-token-exchange-feasibility.md) were resolved and the library contract was committed in [ADR-0014](./docs/02-decisions/0014-api-auth-library-design.md). Audience-at-login (per ADR-0012 NO-GO on RFC 8693 token-exchange).
...
> 7. **🚨 OpenBao `auth/oidc/role/admin` degraded — admin auth locked out (HIGH, surfaced 2026-05-01 post-Fix-after-07)**. CLI `bao login -method=oidc role=admin` returns "Unable to authorize role 'admin' with redirect_uri 'http://localhost:8250/oidc/callback'"; Web UI returns "Invalid role" pre-Keycloak (different code path, same role, suggests deeper degradation than just a missing URI). Phase 7.0.c was marked ✅ but cluster state doesn't match what `infrastructure/openbao/configure-auth-oidc.sh` would have written. Catalogued as **F-CLU-11 (HIGH)** in [`Fix after 07/00-audit-findings.md`](./Fix%20after%2007/00-audit-findings.md#f-clu-11--high--openbao-authoidcroleadmin-degraded--both-cli-and-web-ui-lock-out-post-fix-after-07-discovery-2026-05-01) with full investigation + remediation steps. **Note (root-cause correction):** the operator-flagged `bao bao write` "typo" in the script is **not a typo** — it's a wrapper-function pattern (bash function `bao` that wraps `kubectl exec ... -- env BAO_SKIP_VERIFY=1 BAO_TOKEN=$BAO_TOKEN "$@"` + the `bao` CLI binary inside the pod). All 5 occurrences are correct. The actual regression has a different cause (likely Phase 7.0.c source-edit-only never re-applied, or post-7.0.c overwrite). **Path forward:** investigate live role with `bao read auth/oidc/role/admin` (using bootstrap root token from Shamir-protected laptop custody to bypass OIDC), diff against script, re-apply or patch as needed. **Blocks:** Grafana `client_secret` rotation (deferred until admin auth functional). **Does NOT block:** workload-side JWT-SVID auth (different auth method); existing `BAO_TOKEN` env-vars from prior sessions; Phase 6b-1 (which uses workload SPIFFE-JWT auth, not OIDC); Phase 9 (same).
>
