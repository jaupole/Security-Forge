...
> Last updated: 2026-05-02 (Session 5 — **Phase 7d full prompt-doc + Items 3, 4, 5, 6, 7 ✅ Complete; operator-backlog #4 + #16 closed; new follow-ups #17 + #18 filed.** 7d.1 ships rotate-bff-key.sh + 4 staggered CronJobs + runbook; manual rotation verified. 7d.2 ships ADR-0023 + Path B refresher CronJob; verified end-to-end. **Item 3:** Transit token → -period=720h periodic; ADR-0009 + runbooks updated; #4 closed. **Item 4:** OpenBao metrics → bearer-token auth (Secret must be in ServiceMonitor's ns — discovered during apply); 3 targets `up` post-cutover. **Item 5:** standalone hardened Wazuh agent DaemonSet at `infrastructure/wazuh-agent/` (separate ns w/ PSS=privileged; capabilities-only; hostPID dropped; image digest-pinned). **Item 6:** localfile config for Keycloak + OpenBao pod logs in agent ossec-supplements.xml. **Item 7:** Keycloak client + VSO binding shipped + verified live; `configure-wazuh-oidc.sh` is operator-time switchover. **Open follow-ups (operator-backlog #17, #18):** agent's `client.keys` persistence quirk (~30-60min fix; pre-register + Secret-mount), manager-side custom decoders for OpenBao+Keycloak JSON (~1hr). **Operator-backlog #16** (Phase 5.7 secforge-app rot) re-bootstrapped. **Scheduled remote agent** `trig_01TdbXs13jkY8jyLsfJuHEMM` fires 2026-07-08 for first cron-driven rotation verification.)
...
| 🟨 7d — Rotation + housekeeping batch | **7d.1 + 7d.2 + Items 3, 4, 5, 6, 7 (scaffolding) ✅ Complete 2026-05-02.** Item 7 scaffolding (Keycloak client + VSO bindings + dashboard reconfig script) ready; auth-mode flip is operator-time. Item 5 hardening shipped (separate `wazuh-agent` ns with PSS=privileged, capabilities-only, hostPID dropped, image digest-pinned); known limitation = `client.keys` persistence quirk = enrollment-loop, tracked as Phase 7d.5 follow-up. Item 6 pod-log tailing config in place (`<localfile>` in ossec-supplements.xml); event flow blocked by Item 5 persistence; tracked as Phase 7d.6 follow-up. |
                                ...
                                └─→ Phase 6 (Istio + BFF) ✅
                                      ...
                                      └─→ Phase 7 (Observability) ✅
                                            ...
                                            └─→ 7.1–7.10 (stack) ✅
                                                  ...
                                                  └─→ Phase 7d (Rotation + housekeeping) 🟨 2026-05-02 (7d.1 + 7d.2 + Items 3+4+5+6+7-scaffolding ✅; 7d.5 client.keys persistence + 7d.6 manager decoders deferred as focused follow-ups)
                                                        ↓
