...
| 🟨 7c — Istio SPIRE-as-CA + PeerAuth STRICT | **Unblocked 2026-05-03** — gating 7c.0.1 observability gap (operator-backlog #15) closed: original 2026-05-02 finding was at the wrong log level. ztunnel emits full per-policy + per-clause RBAC trace at `RUST_LOG=trace` (scopes `ztunnel::state` + `ztunnel::rbac`); the admin port `POST :15000/logging?level=trace` toggles it without a pod restart. **Phase 7c proper (1-2 day SPIRE-as-CA cutover + STRICT enablement) not yet started — needs explicit operator window.** |
...
**Status: 🟨 UNBLOCKED 2026-05-03 — gating 7c.0.1 observability gap closed; SPIRE-as-CA cutover + STRICT enablement still TODO and needs an explicit operator window.**
