...
This directory vendors `ileonelperea/wazuh-helm` v1.2.10 (see `.provenance`).
...
**File:** `templates/manager/statefulset.yaml` (manager main container `securityContext.capabilities.add`)
...
namespace per `platform/manifests/wazuh/01-namespace.yaml`) **forbids** `NET_RAW`
...
set the value via `platform/values/wazuh.yaml` instead.

---

## P-002 (2026-05-01) — Remove `workload=wazuh` nodeSelector + matching toleration from cleanup CronJob

**File:** `templates/cleanup/cronjob.yaml`

**Why:** Chart targets EKS Auto Mode node pools labeled `workload=wazuh`. Our
single-node cluster has no such label/taint, so the cleanup pod is
stuck in `Pending` with `0/1 nodes are available: 1 node(s) didn't match Pod's
node affinity/selector`.

**Impact:** cleanup CronJob now schedules anywhere. Acceptable for a single-node
deployment; in cloud edition we'd re-introduce the nodeSelector + matching node label.
...
`platform/values/wazuh.yaml` instead.

**Companion files (NOT patched today):**
- `templates/indexer/cronjob-drift-detection.yaml` and
  `templates/indexer/cronjob-watchdog.yaml` carry the same nodeSelector
