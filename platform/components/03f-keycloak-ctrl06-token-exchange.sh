...
# Keycloak side of that to BOTH realms. The realm-import YAML is ONE-SHOT, so on a
# live/rebuilt cluster these client-scope + mapper edits must land here (idempotent
# — safe to re-run; re-runs are no-ops).
#
# Declarative source of truth stays aligned in
# platform/manifests/keycloak/realms/{secforge-tenants,platform}-realm.yaml — but the
# OPTIONAL client scope below cannot be expressed safely inside the one-shot import (a
# partial `clientScopes:` block can clobber Keycloak's built-in default scopes), so
# the scope itself is OWNED here. The YAMLs carry STE=true + the self-audience
# mappers; this script adds control-aud and removes the legacy static control mappers.
#
# The app clients (member-hub, proposal-forge, business-manager; project-manager only
# in tenants) exist in BOTH realms (Shape B) — a user signing into an app via the
# `platform` realm (staff) gets a platform-issued token, so the exchange must work
# there too. This script therefore applies to secforge-tenants AND platform.
#
# WHAT THIS APPLIES (per realm):
#   - standard.token.exchange.enabled = true on each app client present.
#   - an OPTIONAL client scope `control-aud` carrying an oidc-audience-mapper
#     (included.custom.audience=control), assigned OPTIONAL to each app client.
...
#
# The apps request the scope via @jaupole/ecosystem-auth >= 0.1.10 (controlAudience=
# 'control' + controlExchangeScope='control-aud'; the lib exchanges per the subject
# token's issuing realm, so the same client must be configured in whichever realm the
...
#
# AUTH: the `control-admin` service account (realm-admin in EACH realm) against the
# admin REST API at https://kc.${DOMAIN}. Per-realm client secret in k8s Secret
# control-app-secrets, ns control: KEYCLOAK_TENANTS_ADMIN_CLIENT_SECRET (tenants),
# KEYCLOAK_ADMIN_CLIENT_SECRET (platform). Self-contained — no kcadm cached-config
# prerequisite (unlike 03a/03d). The bootstrap temp-admin is disabled.
#
...
apply_realm() {
  local realm="$1" secret_key="$2"
  local KC="https://kc.${DOMAIN}/admin/realms/${realm}"
  local TOKEN_URL="https://auth.${DOMAIN}/realms/${realm}/protocol/openid-connect/token"

  echo ""
  echo "════════════════════════════════════════════════════════════"
  echo "  CTRL-06 Keycloak apply — realm: ${realm}"
  echo "════════════════════════════════════════════════════════════"

  echo ">>> [00] Acquire control-admin token (${secret_key})"
  local SECRET TOKEN auth SCOPE_ID HAS_MAP CID STE MID
  SECRET=$(kubectl get secret control-app-secrets -n control -o jsonpath="{.data.${secret_key}}" | base64 -d)
  TOKEN=$(curl -s -X POST "$TOKEN_URL" \
    -d grant_type=client_credentials -d client_id=control-admin -d client_secret="$SECRET" \
    | jq -r .access_token)
  [ -n "$TOKEN" ] && [ "$TOKEN" != null ] || { echo "FATAL: no control-admin token for ${realm}" >&2; return 1; }
  ...
  for c in "${APP_CLIENTS[@]}"; do
    ...
    if [ -z "$CID" ]; then echo ">>> [$c] not in $realm — skipping"; continue; fi
    echo ">>> [$c] ($CID)"
...
TARGET="${1:-all}"
case "$TARGET" in
  tenants)  apply_realm secforge-tenants KEYCLOAK_TENANTS_ADMIN_CLIENT_SECRET ;;
  platform) apply_realm platform         KEYCLOAK_ADMIN_CLIENT_SECRET ;;
  all)
    apply_realm secforge-tenants KEYCLOAK_TENANTS_ADMIN_CLIENT_SECRET
    apply_realm platform         KEYCLOAK_ADMIN_CLIENT_SECRET
    ;;
  *) echo "Unknown target '$TARGET'. Valid: tenants, platform, all (default)" >&2; exit 2 ;;
esac

echo ""
echo "════════════════════════════════════════════════════════════"
echo "  CTRL-06 Keycloak state applied. Verify per realm: a default"
echo "  app token has no aud=control; an exchange with"
echo "  scope=control-aud yields aud=control; Control require rejects"
echo "  the former (401) and accepts the latter."
echo "════════════════════════════════════════════════════════════"
